...
object Main extends App {
  ...
  case class Curr(max: Int, min: Int)

  def parseInts(line: String): Seq[Int] = line.split("[ \t]").map(Integer.parseInt)

  def lineChecksum(line: Seq[Int]): Int = line.foldLeft(Curr(line.head, line.head)) {(curr, nextNumber) => Curr(Math.max(curr.max, nextNumber), Math.min(curr.min, nextNumber))} match { case Curr(max, min) => max - min }

  def checksum(input: String): Int = input.lines.map(parseInts).map(lineChecksum).sum
  ...
  println(checksum(example1))


  //star 1 input
  val star1 = """414 382	1515	319	83	1327	116	391	101	749	1388	1046	1427	105	1341	1590
