...
object day2 extends App {
  ...
  val ids:List[String] = input.lines.toList

  val twice = ids.filter(id => id.exists(c => id.count(x => x==c) == 2))
  val thrice = ids.filter(id => id.exists(c => id.count(x => x==c) == 3))

  val checksum = twice.size*thrice.size
  println(checksum)

  val input2 = ids

  def sameCharacterCount(s1:String, s2:String) = s1.zip(s2).count({ case (c1, c2) => c1.equals(c2)})
  println(sameCharacterCount("hoi", "hai"))
  val differsByOne = input2.filter(id => input2.exists(id2 => sameCharacterCount(id, id2) == id.length-1))
  println(differsByOne)
