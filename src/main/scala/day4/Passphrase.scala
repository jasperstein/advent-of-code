...
object Passphrase extends App {
  def full_list(inputs: String) = inputs.lines

  def isValidPassphrase(phrase: String) = {
    val words = phrase.split("\\s+")
    words.length == words.distinct.length
  }

  //examples
  println(isValidPassphrase("aa bb cc dd ee"))
  println(isValidPassphrase("aa bb cc dd aa"))
  println(isValidPassphrase("aa bb cc dd aaa"))


  val star1 = """bdwdjjo avricm cjbmj ran lmfsom ivsof
